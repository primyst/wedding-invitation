...
function PuzzlePiece({ label, side, onSolved }: PuzzlePieceProps) {
  ...
  return <motion.button type="button" aria-label={`Move ${label} heart piece`} drag dragMomentum={false} onDragStart={() => setDragged(true)} onDragEnd={(_, info) => { setDragged(false); if (Math.abs(info.offset.x) > 70) onSolved(); }} animate={{ rotate: dragged ? (side === "left" ? -6 : 6) : 0, scale: dragged ? 1.08 : 1 }} className={`puzzle-piece puzzle-piece-${side}`} whileTap={{ scale: 1.04 }}><span>{label}</span></motion.button>;
}
